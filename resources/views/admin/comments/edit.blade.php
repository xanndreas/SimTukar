    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" type="text" name="content" id="content" value="{{ old('content', $comment->content) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $comment->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('report_count') ? 'is-invalid' : '' }}" type="text" name="report_count" id="report_count" value="{{ old('report_count', $comment->report_count) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('berita') ? 'is-invalid' : '' }}" name="berita_id" id="berita_id">
                    @foreach($beritas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('berita_id') ? old('berita_id') : $comment->berita->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
